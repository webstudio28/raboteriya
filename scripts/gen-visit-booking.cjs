...
const out = path.join(__dirname, "..", "src", "assets", "js", "visit-booking-modal.js");
const discount = path.join(__dirname, "..", "src", "assets", "js", "discount-modal.js");

const visit = String.raw`(function () {
  ...
  function resetForm() {
    ...
    setDateSelection(null); setTimeSelection(null); closePickers(); hideError(); showFormView(); renderCalendar();
  }
  function openModal() { lastFocus = document.activeElement; resetForm(); dialog.showModal(); lockBodyScroll(); var f = document.getElementById("booking-name"); if (f) f.focus(); }
  ...
  document.querySelectorAll(".js-booking-modal-open").forEach(function (el) { el.addEventListener("click", function (e) { e.preventDefault(); openModal(); }); });
  ...
  dialog.addEventListener("close", function () { unlockBodyScroll(); closePickers(); resetForm(); if (lastFocus && lastFocus.focus) lastFocus.focus(); lastFocus = null; });
  if (submitBtn) form.addEventListener("submit", function (e) {
    ...
    var subjectLine = "Raboteriya — " + name + " booked a visit (" + visitDateStr + " " + selectedTime + ")";
    var adminMessage = "Raboteriya — visit booking request (pricing modal)\n\nName: " + name + "\nEmail: " + email + "\nPhone: " + phone +
...
const discountSrc = fs.readFileSync(path.join(__dirname, "discount-modal.source.txt"), "utf8");

fs.mkdirSync(path.dirname(out), { recursive: true });
fs.writeFileSync(out, visit);
fs.writeFileSync(discount, discountSrc);
console.log("Wrote", out, "and", discount);
